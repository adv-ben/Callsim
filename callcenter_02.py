...
class logdata():
    ...
    def doLogs(self, cc, time):
        self.log.append([time, cc.cq.q.qsize()])
        print(self.log)


def plotting(l, plots_to_display):
    if("queue size" in plots_to_display):
        time = []
        qlen = []
        for t, q in l.log:
            time.append(t)
...
def main():
    ...
    while(not eventQueue.empty()):
        ...
        l.doLogs(cc, eventTime)

    print("Simulation Finished")

    plots_to_display = ["queue size"]
    plotting(l, plots_to_display)
